...
{
    double x = UtilityMath::Deg2Rad(static_cast<double>(radX));
    double y = UtilityMath::Deg2Rad(static_cast<double>(radY));
    double z = UtilityMath::Deg2Rad(static_cast<double>(radZ));
    return Euler(x,y,z);
}
...
Quaternion Quaternion::AngleAxis(float _deg, VECTOR axis)
{
    double rad = UtilityMath::Deg2Rad(static_cast<double>(_deg));
...
MATRIX Quaternion::ToMatrix(const Quaternion& q)
{
    ...
    FLOAT4 fq = { static_cast<float>(q.x),
...
Quaternion Quaternion::LookRotation(const VECTOR& dir, const VECTOR& up)
{
    ...
    VECTOR norDir = UtilityMath::VNormalize(dir);
    VECTOR right = UtilityMath::VNormalize(VCross(up, norDir));
...
Quaternion Quaternion::Slerp(const Quaternion& from, const Quaternion& to, float _term)
{
	float term = std::clamp(_term, 0.0f, 1.0f);
    return SlerpUnclamped(from, to, term);
...
Quaternion Quaternion::SlerpUnclamped(Quaternion a, Quaternion b, float t)
{
    ...
    float cosHalfAngle = static_cast<float>(a.w * b.w) + VDot(a.xyz(), b.xyz());
...
void Quaternion::ToAngleAxis(float* angle, VECTOR* axis)
{
	...
	*angle = (2.0f * acosf(static_cast<float>(this->w))); // angle
	...
	float den = sqrtf(1.0f - static_cast<float>(this->w * this->w));
	if (den > 0.0001f)
	{
		//axis = q->xyz / den;
		VECTOR vec = this->xyz();
