...
float UtilityMath::Clamp(float v, float min, float max)
{
	v = ((v > max) ? max : v);
	v = ((v < min) ? min : v);
    return v;
...
bool UtilityMath::IsHitPointSphere(const VECTOR& pos1, const VECTOR& pos2, float radius)
{
    // 球体同士の衝突判定
    bool ret = false;
...
bool UtilityMath::IsHitSpheres(const VECTOR& pos1, float radius1, const VECTOR& pos2, float radius2)
{
    // 球体同士の衝突判定
    bool ret = false;
...
bool UtilityMath::IsHitSphereCapsule(
    ...
    const VECTOR& capPos1, const VECTOR& capPos2, float capRadius)
{
    ...
    // カプセル繋ぎのベクトルの長さを取る
    float len = UtilityMath::MagnitudeF(cap1to2);
    ...
    // 球体同士の当たり判定
    if (UtilityMath::IsHitSpheres(centerPos, capRadius, sphPos, sphRadius))
    {
        ret = true;
    }
    else
...
bool UtilityMath::Equals(const Vector2& v1, const Vector2& v2)
{
...
bool UtilityMath::EqualsVZero(const Vector2& v1)
{
    const VECTOR& v2 = VECTOR_ZERO;
    if (v1.x == v2.x && v1.y == v2.y)
    {
...
void UtilityMath::DrawLineDir(const VECTOR& pos, const VECTOR& dir, int color, float len, float _sphereSize)
{
...
MATRIX UtilityMath::GetMatrixRotateXYZ(const VECTOR& euler)
{
...
MATRIX UtilityMath::Multiplication(const MATRIX& child, const MATRIX& parent)
{
    return MMult(child, parent);
}

MATRIX UtilityMath::Multiplication(const VECTOR& childEuler, const VECTOR& parentEuler)
{
    // モデルの回転行列
