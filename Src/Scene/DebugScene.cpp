...
void DebugScene::PlaceDebugPoint(void)
{
	...
	if (input_.IsTrgDown(InputManager::TYPE::DEBUG_POINT_SET))
	{
		// マウス座標の取得
		...
		auto hit = MV1CollCheck_Line(colliderModel->GetFollow()->modelId, -1,
									 camera->GetPos(),
									 worldPos);
	...
	if (input_.IsTrgDown(InputManager::TYPE::DEBUG_POINT_DELETE))
	{
		if (points_.size() > 0)
		{
			points_.pop_back();
