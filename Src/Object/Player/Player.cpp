...
void Player::UpdateGravityRotation(void)
{
	...
	if (playerNo_ == PLAYER_NO::P1)
	{
		...
		{
		case GRAVITY_DIR::UP:    rotZ = 180.0f;  break; // 「上」で左を向くなら、-90度して上に合わせる
		case GRAVITY_DIR::DOWN:  rotZ = 0.0f;   break; // 「下」で右を向くなら、90度して下に合わせる
		case GRAVITY_DIR::LEFT:  rotZ = 90.0f;  break; // 左右逆転を直す
		case GRAVITY_DIR::RIGHT: rotZ = -90.0f;    break; // 左右逆転を直す
		default: return;
		}
...
void Player::InitTransform(void)
{
	...
	if (playerNo_ == PLAYER_NO::P2
		&& stageType_ == static_cast<int>(STAGE_TYPE::GRAVITY)) 
	{
		localRotZ = 180.0f;
	}
...
void Player::ProcessMove(void)
{
	...
	//GRAVITYステージ（新しい重力操作）
	else if (stageType_ == static_cast<int>(STAGE_TYPE::GRAVITY)) 
	{
		// 入力による重力方向の切り替え（他方のプレイヤーには影響しない）
		if (playerNo_ == PLAYER_NO::P1)
		{
			if (input_.IsTrgDown(InputManager::TYPE::GRAVITY_MOVE_LEFT, Input::JOYPAD_NO::PAD1))
				{ curGravityDir_ = GRAVITY_DIR::LEFT; }

			if (input_.IsTrgDown(InputManager::TYPE::GRAVITY_MOVE_RIGHT, Input::JOYPAD_NO::PAD1))
				{ curGravityDir_ = GRAVITY_DIR::RIGHT; }
		}
		else if (playerNo_ == PLAYER_NO::P2)
		{
